...
// ── HANDLER PRINCIPAL (Background Function — sin límite de tiempo) ────────────
exports.handler = async function(event) {
  ...
  try {
    ...
    // Si Claude falla, notificar a Warren y salir
    await fetch(RESEND_API, {
      ...
      body: JSON.stringify({
        from: FROM_EMAIL,
        to: [WARREN_EMAIL],
        subject: `⚠️ Error crítico — Diagnóstico Estratégico`,
        html: `<p style="font-family:sans-serif;font-size:14px;">Claude falló al generar el diagnóstico.<br><br>Error: ${err.message}<br><br>Usuario: ${userData?.nombre} ${userData?.apellidos} — ${userData?.correo}</p>`
      })
    }).catch(() => {});
    return {
      statusCode: 502,
    ...
    const warrenRes = await fetch(RESEND_API, {
      ...
      body: JSON.stringify({
        ...
        html: emailWarren({
          ...
          whatsapp: userData.whatsapp, puntaje: score, diagnosticoHtml
        })
      })
    });
    if (!warrenRes.ok) {
    ...
    const prospectoRes = await fetch(RESEND_API, {
      ...
      body: JSON.stringify({
        ...
        html: emailProspecto({
          nombre: userData.nombre, apellidos: userData.apellidos,
          empresa: userData.empresa, puesto: userData.puesto,
          pais: userData.pais, puntaje: score, diagnosticoHtml
        })
      })
    });
    if (!prospectoRes.ok) {
  ...
  // ── 6. Alerta si hubo errores secundarios ──────────────────────────────────
      ...
      await fetch(RESEND_API, {
        ...
        body: JSON.stringify({
          ...
          html: `<p style="font-family:sans-serif;font-size:14px;color:#111;">
            <strong>Errores en el sistema del Diagnóstico Estratégico:</strong><br><br>
            <ul>${sideEffectErrors.map(e => `<li>${e}</li>`).join('')}</ul>
            <br>El diagnóstico fue generado y enviado al usuario.<br><br>
            <a href="https://app.netlify.com/projects/inquisitive-jelly-3090da/logs/functions">Ver logs en Netlify</a>
          </p>`
        })
      });
    } catch (alertErr) {
      console.error('[analyze] Alerta falló:', alertErr.message);
    }
  }
